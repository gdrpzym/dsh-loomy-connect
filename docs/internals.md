...
设置卡片在此基础上分为「积分」与「模型」两个标签页：登录态与脱敏手机号在卡片顶部的状态行展示（圆点 + 文案，不进标签页）；「积分」列出永久 / 每日赠送余额；「模型」列出每个模型的名称、类型（对话 / 图像）、倍率与优惠绿胶囊，以及上下文窗口（`contextWindow`，Loomy 的清单提供该字段时才显示）。标签页是卡片内的受控切换，不依赖任何官方 Tab 原语。
