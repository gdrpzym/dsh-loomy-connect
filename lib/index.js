...
function defaultLoomyLocalStorageCandidates() {
	...
	const candidates = [join(homedir(), "Library", "Application Support", "loomy", "Local Storage", "leveldb"), join(homedir(), ".config", "loomy", "Local Storage", "leveldb")];
...
function xdgConfigHome() {
	...
	return raw !== void 0 && raw.trim() !== "" ? raw : join(homedir(), ".config");
}
/**
...
function defaultLoomyAuthCandidates() {
	const appData = process.env.APPDATA;
	const candidates = [join(homedir(), "Library", "Application Support", "loomy", "auth-session.json"), join(xdgConfigHome(), "loomy", "auth-session.json")];
...
//#endregion
export { Config, LOOMY_API_BASE, LOOMY_AUTH_SESSION_KEY, LOOMY_POINTS_KEY, LOOMY_PROVIDER, LOOMY_SETTINGS_NS, LOOMY_STATUS_PATH, LoomyCatalog, LoomyUpstreamClient, apply, createLoomyAdapter, createLoomyShim, defaultLoomyAuthPath, defaultLoomyConfigPath, defaultLoomyLocalStorageDir, extractLoomyAuthSessions, extractLoomyPoints, inject, loomyStatusHandler, loomyWebStatus, loopbackHost, loopbackOrigin, name, newestLoomyPoints, parseLoomyAuth, parseLoomyModels, parseLoomyPointsRecord, prepareLoomyBody, readLoomyCredential, readLoomyPoints, readLoomySessionFromStorage, registerLoomyStatusRoute, resetLoomyPointsCache, resolveAuthFile };
